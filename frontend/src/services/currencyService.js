...
const CURRENCY_CONFIGS = {
  ...
  EUR: { symbol: '€', precision: 2, pattern: '!#', decimal: '.', separator: ',' },
  GBP: { symbol: '£', precision: 2, pattern: '!#', decimal: '.', separator: ',' },
  NGN: { symbol: '₦', precision: 2, pattern: '!#', decimal: '.', separator: ',' },
  GHS: { symbol: '₵', precision: 2, pattern: '!#', decimal: '.', separator: ',' },
  KES: { symbol: 'KSh', precision: 2, pattern: '!#', decimal: '.', separator: ',' },
  ZAR: { symbol: 'R', precision: 2, pattern: '!#', decimal: '.', separator: ',' },
  EGP: { symbol: 'E£', precision: 2, pattern: '!#', decimal: '.', separator: ',' },
  MAD: { symbol: 'MAD', precision: 2, pattern: '# !', decimal: '.', separator: ',' },
  TND: { symbol: 'TND', precision: 3, pattern: '# !', decimal: '.', separator: ',' },
  DZD: { symbol: 'DZD', precision: 2, pattern: '# !', decimal: '.', separator: ',' },
  CAD: { symbol: 'C$', precision: 2, pattern: '!#', decimal: '.', separator: ',' },
  AUD: { symbol: 'A$', precision: 2, pattern: '!#', decimal: '.', separator: ',' }
};
...
class CurrencyService {
  ...
  detectUserCurrency() {
    try {
      ...
      if (this.isValidCurrency(detectedCurrency)) {
        ...
        console.log('Detected currency from timezone:', detectedCurrency, 'from timezone:', timezone);
      } else {
  ...
  formatPrice(price, currencyCode = this.userCurrency) {
    ...
    if (!config) {
      ...
      console.log('No config found for currency:', currencyCode, 'using fallback formatting');
   ...
   */
  async convertAndFormatPrice(price, fromCurrency, toCurrency = this.userCurrency) {
    // Ensure currencies are valid
    if (!fromCurrency || typeof fromCurrency !== 'string') {
      fromCurrency = 'XAF';
    }
    ...
    const convertedPrice = await this.convertPrice(price, fromCurrency, toCurrency);
  ...
  getCurrencyInfo(currencyCode = this.userCurrency) {
    return this.availableCurrencies.find(c => c.code === currencyCode.toUpperCase()) || {
      code: currencyCode,
      symbol: currencyCode,
      name: currencyCode,
      isActive: true
  ...
  isValidCurrency(currencyCode) {
    return currencyCode && this.availableCurrencies.some(c => c.code === currencyCode.toUpperCase());
  }
