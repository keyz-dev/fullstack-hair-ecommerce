...
} = require("../utils/returnFormats/paymentMethodData");
...
const getAllPaymentMethods = async (req, res, next) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        paymentMethods: paymentMethods.map(formatPaymentMethodData),
      });
  } catch (err) {
    next(err);
  }
  ...
  try {
    ...
    res.status(200).json({
      ...
      message: isConfigured
        ? "Payment method is properly configured"
        : "Payment method requires configuration",
    });
  } catch (err) {
    next(err);
  }
